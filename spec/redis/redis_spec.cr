...
describe Alumna::Redis do
  it "pings a live server from a String URI" do
    holder = must_redis(Alumna::Redis.new(REDIS_URL))
    holder.ping.should eq("PONG")
    holder.close
  end

  it "pings a live server from a URI object" do
    holder = must_redis(Alumna::Redis.new(URI.parse(REDIS_URL)))
    holder.ping.should eq("PONG")
  ...
  it "builds from_uri with a URI and with a String" do
    from_obj = must_redis(Alumna::Redis.from_uri(URI.parse(REDIS_URL), prefix: "from-uri:"))
    from_obj.ping.should eq("PONG")
    from_obj.prefix.should eq("from-uri:")
    from_obj.close

    from_str = must_redis(Alumna::Redis.from_uri(REDIS_URL))
  ...
  it "builds from_env with REDIS_URL and with a custom name" do
    ...
    begin
      holder = must_redis(Alumna::Redis.from_env)
      holder.ping.should eq("PONG")
      holder.close
    ensure
      if old
    ...
    begin
      holder = must_redis(Alumna::Redis.from_env("ALUMNA_REDIS_SPEC", prefix: "env:"))
      holder.ping.should eq("PONG")
  ...
  it "raises ArgumentError when from_env is missing or empty" do
    ...
    expect_raises(ArgumentError, "Missing environment variable ALUMNA_REDIS_MISSING") do
      Alumna::Redis.from_env("ALUMNA_REDIS_MISSING")
    end

    ENV["ALUMNA_REDIS_EMPTY"] = ""
    begin
      expect_raises(ArgumentError, "Missing environment variable ALUMNA_REDIS_EMPTY") do
  ...
  it "raises ArgumentError for an empty URL" do
    expect_raises(ArgumentError, "Redis URL must not be empty") do
      Alumna::Redis.new("")
    end
  end

  it "raises ArgumentError for a URI that does not parse" do
    expect_raises(ArgumentError) do
      Alumna::Redis.new("http://example.com:abc")
    end
    ex = expect_raises(ArgumentError) do
      Alumna::Redis.new("redis://user:secret@host:abc")
    end
  ...
  it "uses default port prefixes and joins a global prefix" do
    ...
    holder = must_redis(Alumna::Redis.new(
      REDIS_URL,
      prefix: "shop:",
      cache_prefix: "c:",
      session_prefix: "s:",
  ...
  it "returns Error without URI userinfo when the server is down" do
    result = Alumna::Redis.new(dead_url)
  ...
  it "returns Error on ping when the pool did not connect at new" do
    holder = must_redis(Alumna::Redis.new(dead_url(lazy: true)))
    ping = holder.ping
    ping.should be_a(Alumna::Redis::Error)
    if ping.is_a?(Alumna::Redis::Error)
      ping.message.includes?("secret").should be_false
    end
    holder.close
  end
  ...
  it "returns Error without URI userinfo when Cluster is down" do
    result = Alumna::Redis.new(dead_url, cluster: true)
    result.should be_a(Alumna::Redis::Error)
    if result.is_a?(Alumna::Redis::Error)
      result.message.includes?("secret").should be_false
      result.message.includes?("user:").should be_false
...
describe "Alumna::Redis cluster" do
  it "pings a live Cluster from a String URI" do
    ...
    holder = must_redis(Alumna::Redis.new(url, cluster: true))
    holder.ping.should eq("PONG")
  ...
  it "pings a live Cluster from a URI object" do
    url = require_cluster_url
    holder = must_redis(Alumna::Redis.new(URI.parse(url), cluster: true))
    holder.ping.should eq("PONG")
    holder.close
  end
  ...
  it "builds from_uri and from_env with cluster: true" do
    url = require_cluster_url
    from_obj = must_redis(Alumna::Redis.from_uri(URI.parse(url), prefix: "from-uri-c:", cluster: true))
    from_obj.ping.should eq("PONG")
    ...
    begin
      holder = must_redis(Alumna::Redis.from_env("ALUMNA_REDIS_CLUSTER_SPEC", prefix: "env-c:", cluster: true))
      holder.ping.should eq("PONG")
